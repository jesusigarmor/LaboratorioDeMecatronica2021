...
def orientate (xgoal, ygoal):
	...
	while(True):
		...
		if desired_angle_goal < 0:
			desired_angle_goal = desired_angle_goal + 2*abs(math.pi)
			dtheta = abs(desired_angle_goal) - theta
		        #print ('theta=', theta)			
		...
		if abs(desired_angle_goal - theta) < abs((desired_angle_goal + 2*abs(math.pi)) - theta):	
			dtheta = desired_angle_goal - theta
		#	print ('dtheta=', dtheta)
		else:
			dtheta = desired_angle_goal + 2*abs(math.pi) - theta
		#	print ('dtheta=', dtheta)        
		#
		if (abs(dtheta) < 0.005):
...
~~~
    ...
    orientate(10.0,1.0)
		...
		orientate(10.0,10.0)
		time.sleep(0.5)
		go_to_goal(10.00,10.0)
		time.sleep(0.5)
		
		orientate(1.0,10.0)
		time.sleep(0.5)
		go_to_goal(1.00,10.0)
		time.sleep(0.5)
